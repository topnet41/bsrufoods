...
class _NotiMenuState extends State<NotiMenu> {
  ...
  confirmOrder()async{
      await firestore.collection("orders").doc(widget.orderpath).update({"staOrder":true});
      dataShop["tokenUser"].forEach((value) {
         http.get("https://apibsrufood.000webhostapp.com/apiNotification.php?isAdd=true&token=$value&title=${dataShop["username"]}&body=ออเดอร์ยืนยันแล้ว");
      });
        ...
        body: SingleChildScrollView(
            ...
            children: [
              Container(
                ...
                child: dataShop.length < 1
                    ...
                    : Column(
                      ...
                      child: widget.order["cash"] == "โอนเงิน" ? RaisedButton(
              ...
              Container(
                ...
                child: Center(child: Text("เวลารับ : ${widget.order["time"]== null ? 'ไม่ระบุ' : widget.order["time"]}  สถานะ : "+
              ...
              ),Text("รายการอาหาร",style: TextStyle(fontSize: 24),),
              Container(
                height: MediaQuery.of(context).size.height*0.35,
                child: ListView.builder(shrinkWrap: true,itemCount: widget.order["detail"].length,itemBuilder: (context,index){
                  return Container(
                    padding: EdgeInsets.all(15),
                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,children: [
                      Container(width: 135,child: Text("${index+1}. ${widget.order["detail"][index]["name"]}")),
                      Text("x${widget.order["detail"][index]["count"]}"),
                      Text("${widget.order["detail"][index]["price"]*widget.order["detail"][index]["count"]} บาท")
                    ],),
                  );
                }),
              ),
              Container(
                ...
                child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text("รวมเป็นเงิน",style: TextStyle(fontSize: 18),),
                        Text("$total",style: TextStyle(fontSize: 18),),
                      ],
                    ),
              ),
             !widget.order["status"]["staOrder"] ? SizedBox(
